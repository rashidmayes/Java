...
public class WAPPushCauseway extends Causeway
{
    private Preferences preferences;
    
    
    public WAPPushCauseway()
    {
        preferences = new Preferences("comp.comm.wappush");
    }
    ...
    {		
		...
		try
		{
			...
			String url = preferences.get("wappush.url","https://api.clickatell.com/mms/si_push");
			String queryString = "api_id=" + preferences.get("clickatel.appid") + "&to=" + message.getTo() + "&from=" + message.getFrom() + "&user=" + preferences.get("clickatel.user") + "&password=" + preferences.get("clickatel.password") 
			+ "&si_url=" + URLEncoder.encode(String.valueOf(message.getContent()),"UTF8")
			+ "&si_id=" + new UID().hashCode()
			+ "&si_created=" + Dates.format(new Date(),"yyyy-MM-dd'T'HH:mm:ss'Z'")
			+ "&si_expires=" + Dates.format(new Date(System.currentTimeMillis()+(1000*60*60*24*31)),"yyyy-MM-dd'T'HH:mm:ss'Z'")
			+ "&si_action=signal-low"
			+ "&si_text=" + URLEncoder.encode(Strings.ifNull(message.getHeaders().get("text"),"Hostj2me.com message"),"UTF8");
